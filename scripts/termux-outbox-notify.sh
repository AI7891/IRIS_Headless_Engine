...
fetch_packages() {
  curl -fsS --max-time 15 "$IRIS_URL/api/outbox?status=Exported" 2>/dev/null \
    | jq -r '
        group_by(.packageId)[]
...
while true; do
  ...
  if [ -z "$packages" ]; then
    ...
    curl -fsS --max-time 15 "$IRIS_URL/healthz" >/dev/null 2>&1 \
      || echo "$ts  API unreachable (codespace asleep?), retrying"
  else
    while IFS=$'\t' read -r pkg count hook ref; do
      [ -z "$pkg" ] && continue
